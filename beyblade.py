...
MATCH_FILE = "current_matches.csv"             # 7人單循環賽程檔案
...
main_mode = st.sidebar.radio("選擇要管理的賽制：", ["個人賽 (7人單循環+4強)", "雙人團體賽 (獨立區)"])

# ==========================================
# 4. 模式一：個人賽 (7人單循環 + 4強淘汰賽)
# ==========================================
if main_mode == "個人賽 (7人單循環+4強)":
    st.title("💥 三重盃 個人賽（7人單循環預賽 ➡️ 4強淘汰賽）")
    ...
    with tabs[0]:
        ...
        if is_admin:
            if len(df_reg) == 7 and (df_reg["編號"] == 0).all():
                st.write("---")
                if st.button("🎲 滿 7 人！盲抽生成 1~7 號編號並初始化賽程", type="primary", use_container_width=True):
                    nums = list(range(1, 8))
                    random.shuffle(nums)
                    df_reg["編號"] = nums
                    save_registrations(df_reg)
                    ...
                    # 初始化 21 場對戰紀錄
                    matches_init = []
                    for idx, (p1_id, p2_id) in enumerate(SCHEDULE_21, 1):
                        matches_init.append({
                            "場次": idx,
                            "選手A_編號": p1_id,
                            "選手B_編號": p2_id,
                    ...
                    st.success("🎉 7 人抽籤成功！預賽 21 場賽程已自動生成！")
                    st.rerun()
    ...
    # --- Tab 2: 7人單循環賽程控制台 ---
    with tabs[1]:
        st.header("⚔️ 預賽：7人單循環對戰控制台")
        
        if df_matches is None or (df_reg["編號"] == 0).all():
            st.warning("⏳ 需先在「選手登記」分頁集滿 7 人並進行【盲抽編號】後，才能開始單循環賽！")
        else:
            completed_count = sum(1 for w in df_matches["勝者_編號"] if w != 0)
            st.progress(completed_count / 21, text=f"預賽進度：{completed_count} / 21 場")
            
            # 場次選擇器
            selected_match_idx = st.number_input("選擇對戰場次：", min_value=1, max_value=21, value=min(completed_count + 1, 21), step=1)
