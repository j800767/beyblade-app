...
def render_pk_section(rank_target, candidates, player_map):
    st.error(
        ...
        f" {', '.join([f'{p}號 {player_map[p]}' for p in candidates])}"
    )
    st.info(
...
with main_tab1:
    ...
    with tab4:
        ...
        if df_swiss is None or total_p < 30:  # 5輪 * 6場 = 30場完賽
            ...
            if (
                len(candidates_4th) > 1
                and "selected_rank_4" not in st.session_state
            ):
                ...
                st.success(
                    "🎉 預賽晉級四強名單："
                    f" {', '.join([f'{p}號 {player_map[p]}' for p in final_4])}"
                )
                ...
                if df_finals is None:
                    ...
                    with col_sfa:
                        ...
                        if is_admin:
                            ...
                            st.write(
                                f"勝者：`{sf_a_w if sf_a_w else '未決定'}`"
                            )
                    ...
                    with col_sfb:
                        ...
                        if is_admin:
                            ...
                            st.write(
                                f"勝者：`{sf_b_w if sf_b_w else '未決定'}`"
                            )
