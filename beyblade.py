...
with tab2:
    ...
    # 側邊欄抽籤調整與【新增自動抽籤】
    # ════════════════════════════════════════════════════════════
    st.sidebar.header("🎲 現場分組抽籤系統")
    
    # 🌟 新增功能：一鍵隨機抽籤按鈕
    if st.sidebar.button("💥 開始現場隨機抽籤！", type="primary"):
        # 抓取目前實際登記的人，如果不滿 6 人，就用待定選手補滿 6 個
        pool = raw_players.copy()
        while len(pool) < 6:
            pool.append(f"待定選手_{len(pool)+1}")
        pool = pool[:6]
        ...
        save_scores()
        st.sidebar.success("🎉 抽籤完成！名單已隨機打亂更新！")
        st.rerun()
    ...
    if st.sidebar.button("🔄 重設大賽（比分歸零）"):
